package ru.androidschool.intensiv.data.movidetail
...
fun convertFromMovieToDetail(dto: MovieDetail): MovieDetailDb {
...
fun convertFromDetailToMovie(details: DetailMovieAndGenres): Movies {
    return Movies(
...
fun convertFromGenresTo(ownerId: Int, ganres: List<Genre>): List<GenreDb> {
    return ganres.map {
        GenreDb(
            id = it.id,
            name = it.name,
